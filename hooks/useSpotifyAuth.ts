...
const CLIENT_ID = Constants.expoConfig?.extra?.EXPO_PUBLIC_SPOTIFY_CLIENT_ID || "";
      ...
      if (result.type === "success") {
        ...
        try {
          ...
          if (!request.codeVerifier) {
            throw new Error("PKCE code verifier is missing from the AuthRequest. Cannot complete login.");
          }
          const tokenResponse = await exchangeCodeForToken(code, redirectUri, request.codeVerifier);
          ...
          try {
            ...
            try {
              ...
              console.error("Failed to initialize protection mechanism:", initError);
